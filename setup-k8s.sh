#!/bin/bash
# $Id: setup-k8s.sh 412 2021-07-26 16:04:49Z bpahlawa $
# initially captured from Microsoft website
# $Author: bpahlawa $
# Modified by: bpahlawa
# $Date: 2021-07-27 00:04:49 +0800 (Tue, 27 Jul 2021) $
# $Revision: 412 $
...
EOF
   ...
   for MNTPOINT in `df | awk '{print $NF}' | egrep "docker|kubelet|\/vol"`
   do
       umount $MNTPOINT
   done
...
install_pkg_debian()
{
   ...
   then
      ...
      if [ `cat /etc/apt/sources.list | grep "ubuntu\/18.04" | grep -v "^#.*" | wc -l` -eq 0 ]
      then
         add-apt-repository "$(wget -qO- https://packages.microsoft.com/config/ubuntu/18.04/prod.list)"
      fi
   else
      if [ `cat /etc/apt/sources.list | grep "ubuntu\/16.04" | grep -v "^#.*" | wc -l` -eq 0 ]
      then
         add-apt-repository "$(wget -qO- https://packages.microsoft.com/config/ubuntu/16.04/prod.list)"
      fi
   fi
...
EOF
   ...
   *)  apt-get update -q
   ...
   systemctl stop docker
   [[ $? -ne 0 ]] && echo "Failed to stop docker...exiting..." && exit 1
   ...
   then
	 ...
	 if [ $((LFREESIZE+DCURRSIZE+LCURRSIZE)) -lt $((DOCKERMINSIZE+LOCALSTGMINSIZE)) ]
	     ...
	     echo -e "\nThe mount $DOCKERSTORAGE and $LOCALSTORAGE are on the same mountpoint $DMOUNTPOINT !"
             ...
             echo -e "\nMountpoint Requires free space at least $((DOCKERMINSIZE+LOCALSTGMINSIZE)) GB\nAdditional diskspace is required\n"
             exit 1
         else
             echo -e "\nThe mount $DOCKERSTORAGE and $LOCALSTORAGE are on the same mountpoint $DMOUNTPOINT !"
             echo -e "\nFree space: $LFREESIZE GB, current docker size $DCURRSIZE GB and current local-storage size $LCURRSIZE GB"
         ...
         then
             echo -e "\nMountpoint Requires free space at least $LOCALSTGMINSIZE GB\nAdditional diskspace is required\n"
             exit 1
         else
             echo -e "\nMountpoint Requires free space at least $LOCALSTGMINSIZE GB\nThe current free diskspace is sufficient"
         fi
         ...
         then
             echo -e "\nMountpoint Requires free space at least $DOCKERMINSIZE GB\nAdditional diskspace is required\n"
             exit 1
         else
             echo -e "\nMountpoint Requires free space at least $DOCKERMINSIZE GB\nThe current free diskspace is sufficient\n"
         fi
...
reset_only()
{
   ...
   then
      ...
      if [ "$HOST2DELETE" != "" ]
      then
          su - $THEUSER -c "kubectl drain $HOSTNAME --delete-local-data --force --ignore-daemonsets"
